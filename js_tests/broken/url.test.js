...
const assertThrows = (f, ErrorType, message) => {
  try {
    f();
...
const doTest = () => {
  test(function urlParsing() {
    ...
    // expected https: not https
    assertEquals(url.protocol, "https");
    // expected ?foo=bar&baz=12 not foo=bar&baz=12
  ...
  //     );
  //     url.hash = "";
  //     // expect https://foo:bar@baz.qat:8000/qux/quux?foo=bar&baz=12
  //     assertEquals(
  //         url.href,
  ...
  //     url.protocol = "http";
  //     assertEquals(url.href, "http://foo:qux@foo.bar:8080/foo/bar%qat?foo=bar&baz=12#");
  //     url.search = "foo=bar&foo=baz";
  //     assertEquals(url.href, "http://foo:qux@foo.bar:8080/foo/bar%qat?foo=bar&foo=baz#");
  //     // console.log(url.searchParams.getAll("foo"));
  //     // assertEquals(url.searchParams.getAll("foo"), ["bar", "baz"]);
  //     url.username = "foo@bar";
  //     assertEquals(
  //         url.href,
  //         "http://foo%40bar:qux@foo.bar:8080/foo/bar%qat?foo=bar&foo=baz#",
  //     );
  //     url.searchParams.set("bar", "qat");
  //     assertEquals(
  //         url.href,
  //         "http://foo%40bar:qux@foo.bar:8080/foo/bar%qat?foo=bar&foo=baz&bar=qat#",
  //     );
  //     url.searchParams.delete("foo");
  //     assertEquals(url.href, "http://foo%40bar:qux@foo.bar:8080/foo/bar%qat?bar=qat#");
  //     url.searchParams.append("foo", "bar");
  //     assertEquals(
  //         url.href,
  //         "http://foo%40bar:qux@foo.bar:8080/foo/bar%qat?bar=qat&foo=bar#",
  //     );
  // });
  ...
  test(function urlModifyHash() {
    ...
    // expected #%foo%20bar/qat%qux#bar not %foo%20bar/qat%qux#bar
  ...
  test(function urlEncoding() {
    ...
    // expected ?a%20~!@$&*()=:/,;?+%27%22\ not a%20~!@$&*()=:/,;?+%27%22\
    assertEquals(
  ...
  test(function urlBase() {
    // jstz doesn't support this constructor
  ...
  // jstz doesn't implement this behaviour
  // test(function doNotOverridePortIfInvalid() {
  //     const initialPort = "3000";
  //     const url = new URL(`https://jstz.dev:${initialPort}`);
  //     // If port is greater than 2^16 − 1, validation error, return failure.
  //     url.port = `${2 ** 16}`;
  //     assertEquals(url.port, initialPort);
  // });

  // jstz doesn't implement default ports for special schemes
  ...
  // jstz doesn't implement this constructor
  ...
  //     u2.port = 123;
  //     // assertEquals(u1.port, "");
  //     // assertEquals(u2.port, "123");
  // });
