      ...
      flake-utils.lib.eachDefaultSystem (
        system: let
          ...
          in {
            ...
            preBuild = let
              # Vendor rust_deps
              rust_deps = rustPlatform.importCargoLock {
                lockFile = "${old.src}/src/rust_deps/Cargo.lock";
              };
              # Vendor rustzcash_deps
              rustzcash_deps = rustPlatform.importCargoLock {
                lockFile = "${old.src}/src/rustzcash_deps/Cargo.lock";
              };
              # Configure cargo to get dependencies from vendored dir
              setup_vendored_dir = target_dir: vendored_dir: ''
              ...
              '';
            in ''
              ${setup_vendored_dir "src/rust_deps" rust_deps}
              ${setup_vendored_dir "src/rustzcash_deps" rustzcash_deps}
            '';
