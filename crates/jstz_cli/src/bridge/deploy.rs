...
pub struct DeployBridge {
    ...
    /// Total ticket supply - should be 1:1 equivalent with total token supply
    #[arg(long)]
    ...
    #[arg(default_value = None, value_hint = clap::ValueHint::FilePath)]
    pub config_path: Option<PathBuf>,
}

impl DeployBridge {
    pub async fn exec(self) -> Result<ContractKt1Hash> {
        let DeployBridge {
            source,
            tezos_fa_token,
            fa_token_id,
        ...
        let cfg = Config::load_path(config_path.clone()).await?;
