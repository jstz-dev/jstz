use clap::Subcommand;
use fa_deploy::FaDeploy;
use fa_deposit::FaDeposit;
...
pub enum Command {
    ...
    FaDeploy(FaDeploy),
    /// Deposits FA tokens from SENDER to RECEIVER using the
    /// FA token bridge
    FaDeposit(FaDeposit),
}
...
pub async fn exec(command: Command) -> Result<()> {
    match command {
        ...
        Command::FaDeploy(deploy) => deploy.exec(),
        Command::FaDeposit(deposit) => deposit.exec(),
    }
}
