...
// Represents a collection of accounts: users or 'deployments' (aka smart functions)
...
impl Account {
    pub fn address(&self) -> &Address {
        match self {
            ...
            Account::Deployment(deployment) => &deployment.address,
        }
    }
}
...
impl AccountConfig {
    ...
    pub fn set_current_alias(&mut self, alias: Option<String>) -> Result<()> {
        if let Some(alias) = alias.as_ref() {
            ...
            if let Some(Account::Deployment(_)) = self.accounts.get(alias) {
                bail!(
        ...
        match self {
            ...
            AddressOrAlias::Alias(alias) => {
                let account = cfg
                    .accounts
                    .get(alias)
                    .ok_or_else(|| user_error!("User/deployment '{}' not found. Please provide a valid address or alias.", alias))?;
