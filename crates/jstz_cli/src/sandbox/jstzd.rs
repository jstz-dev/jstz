...
async fn run_jstzd() -> Result<Child> {
    let child = Command::new("jstzd").args(["run"]).spawn()?;

    let jstzd_running = retry(70, 1000, || async { is_jstzd_running().await }).await;
    if !jstzd_running {
        bail_user_error!("Sandbox did not launch successfully.")
    }
    Ok(child)
}
