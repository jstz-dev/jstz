...
pub async fn run(options: RunOptions) -> Result<()> {
    ...
    let storage_sync_handle = storage_sync::spawn(
    ...
    axum::serve(listener, router)
        .with_graceful_shutdown(storage_sync_handle)
        .await?;
...
mod test {
    ...
    use crate::{
        config::RuntimeEnv, run, services::utils::tests::mock_app_state,
        storage_sync::tests::make_line, KeyPair, RunMode, RunOptions,
    };
    ...
    fn kills_storage_sync() -> BatchStorageUpdate {
        let mut updates = BatchStorageUpdate::new(1);
        let key = OwnedPath::try_from("/KILL".to_string()).unwrap();
    ...
    async fn server_dies_when_storage_sync_dies() -> anyhow::Result<()> {
        ...
        // server dies after storage sync dies
