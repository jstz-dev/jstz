...
use jstz_core::reveal_data::{PreimageHash, RevealData};
...
use jstz_proto::context::account::{Account, Nonce};
    ...
    match operation {
        Ok(operation) => match operation.content {
            Content::DeployFunction(_) => {
                ...
                // let nonce = get_nonce(rollup_client).await;
