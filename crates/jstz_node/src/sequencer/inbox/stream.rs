...
///
/// Gap-filling, resumable block streaming for the inbox monitor.
///
/// This stream yields block levels in order, automatically handling gaps, source stream errors,
/// checkpoint persistence, and reconnections.
///
/// ## Example
/// ```rust
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            let (next_state, ret) = match *this.state {
                ...
                // Connected to the source stream, yield the next block
                State::Streaming => this.handle_streaming(cx),
                // Awaiting a checkpoint store read to compare with `live`.
...
mod tests {
    ...
    impl CheckpointStore for MockStore {
        ...
        async fn save(&self, level: BlockLevel) -> io::Result<()> {
    ...
    async fn stream_follows_source_stream() {
        ...
        let future = process_stream(&store, stream);
    ...
    async fn stream_handles_checkpoint_error() {
        ...
        let stream = SequentialBlockStream::new(store.clone(), factory);
        let handle = process_stream(&store, stream);
