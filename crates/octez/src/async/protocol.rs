...
impl ProtocolParameterBuilder {
    ...
    fn merge_bootstrap_contracts(
        ...
        if let Some(value) = json.get("bootstrap_contracts") {
            if let Some(existing_contracts) = value.as_array() {
                for contract in existing_contracts {
                    contracts.add_contract(BootstrapContract::try_from(contract)?);
                }
            }
        }
        for contract in self.bootstrap_contracts.contracts() {
            contracts.add_contract(contract.clone());
        }
        json.insert("bootstrap_contracts".to_owned(), Value::from(&contracts));
        Ok(())
    }
    ...
    fn merge_bootstrap_smart_rollups(
        ...
        if let Some(value) = json.get("bootstrap_smart_rollups") {
            if let Some(existing_rollups) = value.as_array() {
                for rollup in existing_rollups {
                    rollups.add_rollup(BootstrapSmartRollup::try_from(rollup)?);
                }
            }
        }
        for rollup in self.bootstrap_smart_rollups.rollups() {
            rollups.add_rollup(rollup.clone());
        }
        json.insert("bootstrap_smart_rollups".to_owned(), Value::from(&rollups));
        Ok(())
    }
}
...
mod tests {
    ...
    fn create_dummy_source_file(
        ...
        if let Some(contracts) = bootstrap_contracts {
            ...
            obj.insert(
                ...
                Value::Array(contracts.iter().map(Value::from).collect::<Vec<Value>>()),
            );
        }
        if let Some(rollups) = bootstrap_rollups {
            ...
            obj.insert(
                ...
                Value::Array(rollups.iter().map(Value::from).collect::<Vec<Value>>()),
            );
        }
    ...
    fn set_bootstrap_contracts() {
        ...
        let contract = contracts.first().unwrap().as_object().unwrap();
        assert!(contract.get("hash").is_none());
        assert_eq!(contract.get("amount").unwrap().as_str().unwrap(), "1000");
        assert_eq!(contract.get("script").unwrap().as_str().unwrap(), "foobar");

        let contract = contracts.last().unwrap().as_object().unwrap();
        assert_eq!(
            contract.get("hash").unwrap().as_str().unwrap(),
            CONTRACT_HASH
        );
        assert_eq!(contract.get("amount").unwrap().as_str().unwrap(), "2000");
        assert_eq!(contract.get("script").unwrap().as_str().unwrap(), "foobar");
    }
    ...
    fn merge_existing_bootstrap_contracts() {
        ...
        let contracts = json
            ...
            .unwrap()
            .iter()
            .map(|v| BootstrapContract::try_from(v).unwrap())
    ...
    fn set_bootstrap_smart_rollups() {
        ...
        let rollups = json
            ...
            .unwrap();
        assert_eq!(rollups.len(), 1);
        let found_rollup = rollups.last().unwrap().as_object().unwrap();
        assert_eq!(
            found_rollup.get("address").unwrap().as_str().unwrap(),
            SMART_ROLLUP_ADDRESS
        );
        assert_eq!(
            found_rollup.get("pvm_kind").unwrap().as_str().unwrap(),
            SmartRollupPvmKind::Riscv.to_string()
        );
        assert_eq!(
            found_rollup.get("kernel").unwrap().as_str().unwrap(),
            "dummy-kernel"
        );
        assert_eq!(
            found_rollup.get("parameters_ty").unwrap().as_str().unwrap(),
            "dummy-params"
        );
    }
    ...
    fn merge_existing_bootstrap_rollups() {
        ...
        let mut rollups = json
            ...
            .unwrap()
            .iter()
            .map(|v| BootstrapSmartRollup::try_from(v).unwrap())
