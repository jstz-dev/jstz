...
use crate::request::ORACLE_LINE_REGEX;
use crate::request::{request_event_from_log_line, OracleRequestEvent};

pub struct Relay {
    tx: broadcast::Sender<OracleRequestEvent>,
}

impl Relay {
    pub fn spawn(log_path: PathBuf) -> Self {
        let (tx, _rx0) = broadcast::channel(1024);

        let mut stream = FilteredLogStream::new(ORACLE_LINE_REGEX.clone(), log_path);
    ...
    pub fn subscribe(&self) -> broadcast::Receiver<OracleRequestEvent> {
        self.tx.subscribe()
    }
}
...
mod tests {
    ...
    async fn next_event(
        rx: &mut broadcast::Receiver<OracleRequestEvent>,
    ) -> Result<OracleRequestEvent> {
    ...
    async fn forwards_single_valid_line() -> Result<()> {
        ...
        let relay = Relay::spawn(path.clone());
    ...
    async fn ignores_noise_lines() -> Result<()> {
        ...
        tokio::spawn(async move {
            append_async(path_clone.clone(), "not an oracle".into(), 10).await?;
