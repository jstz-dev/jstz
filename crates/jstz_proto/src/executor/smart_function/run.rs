...
use crate::runtime::v2::fetch::{
    convert_header_map, process_and_dispatch_request_borrowed, Body,
};
use crate::{
...
async fn execute_v2(
    ...
    let response = process_and_dispatch_request_borrowed(
        hrt,
        tx,
        source,
        ...
        run_operation.body.map(|v| Body::Vector(v)),
    )
    .await;

    let (http_parts, body) = response.to_http_response().into_parts();
