...
use crate::{
    ...
    executor::smart_function::deploy,
    operation::{DeployFunction, OperationHash},
    runtime::{
        v1::{api::ProtocolData, fetch_handler::fetch},
        ParsedCode,
    },
    Result,
};
...
impl SmartFunction {
    ...
    fn create(
        ...
        let deploy_receipt = deploy::execute(
            hrt,
            tx,
            &self.address,
            DeployFunction {
    ...
    fn call(
        ...
        fetch(self_address, operation_hash, request, context)
    }
}
