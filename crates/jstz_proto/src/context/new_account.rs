...
mod test {
    ...
    mod account {
        ...
        fn test_new_account_path() {
            ...
            let long_addr = NewAddress::User(
                PublicKeyHash::from_base58("tz1cD5CuvAALcxgypqBXcBQEA8dkLJivoFjU")
                    .unwrap(),
            );
            assert!(NewAccount::path(&long_addr).is_ok());

            // Test path format
            let path = NewAccount::path(&user_addr).unwrap();
            assert!(path.to_string().starts_with("/jstz_new_account/"));
            assert!(path.to_string().contains(TZ1));

            let path = NewAccount::path(&sf_addr).unwrap();
            assert!(path.to_string().starts_with("/jstz_new_account/"));
        ...
        fn test_balance_operations() {
            ...
            NewAccount::set_balance(&host, &mut tx, &user_addr, u64::MAX).unwrap();
            assert!(matches!(
                NewAccount::add_balance(&host, &mut tx, &user_addr, 1),
                Err(Error::BalanceOverflow)
            ));

            // Test subtracting balance
            NewAccount::set_balance(&host, &mut tx, &user_addr, 100).unwrap();
            assert_eq!(
            ...
            NewAccount::set_balance(&host, &mut tx, &addr1, 100).unwrap();
            NewAccount::set_balance(&host, &mut tx, &addr2, u64::MAX - 50).unwrap();
            assert!(matches!(
                NewAccount::transfer(&host, &mut tx, &addr1, &addr2, 100),
                Err(Error::BalanceOverflow)
            ));
        }
        ...
        fn test_function_code_operations() {
            ...
            assert!(NewAccount::set_function_code(
                &host,
                &mut tx,
                &sf_addr,
                "".to_string()
