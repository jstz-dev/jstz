...
The protocol assumes the preimages are made available to the kernel before processing the `RevealLargePayload` operation. Only authorized injectors can submit `RevealLargePayload` operations (e.g. jstz node).
...
5. Kernel verifies the operation is signed by an authorized injector
