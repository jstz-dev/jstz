...
async fn activate_protocol() {
    ...
    let base_dir = temp_dir.path().to_path_buf();
    let octez_client = OctezClientBuilder::new()
        .set_endpoint(Endpoint::try_from(Uri::from_static("localhost:18732")).unwrap())
        .set_base_dir(base_dir.clone())
        .build()
        .unwrap();
    ...
    let blocks_head_endpoint =
        format!("http://{}/chains/main/blocks/head", "localhost:18732");
    ...
    let health_check_endpoint = format!("http://{}/health/ready", "localhost:18732");
...
async fn spawn_octez_node() -> (octez_node::OctezNode, TempDir) {
    ...
    config_builder
        .set_rpc_endpoint("localhost:18732")
