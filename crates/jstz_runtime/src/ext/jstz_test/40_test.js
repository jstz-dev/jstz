...
import { escapeName } from "ext:jstz_test/40_test_common.js";
...
function wrapInner(fn) {
  ...
  return async function innerWrapped(desc) {
    ...
    if (runningStepDescsWithSanitizers.length > 0) {
      return {
        failed: {
          overlapsWithSanitizers:
            runningStepDescsWithSanitizers.map(getFullName),
        },
      };
    }
...
test.only = function (nameOrFnOrOptions, optionsOrFn, maybeFn) {
...
function usesSanitizer(desc) {
  return desc.sanitizeResources || desc.sanitizeOps;
}
