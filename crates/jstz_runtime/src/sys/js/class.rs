...
pub(crate) fn new_instance<'s>(
    ...
    match maybe_instance {
        ...
        None if tc_scope.has_caught() => Err(tc_exeption_to_runtime_error(&mut tc_scope)),
...
pub(crate) fn tc_exeption_to_runtime_error(
...
pub(crate) fn instance_set(
    ...
    if let Some(true) = this.set(&mut tc_scope, property_name.into(), value) {
        ...
        Err(tc_exeption_to_runtime_error(&mut tc_scope))
    } else {
