...
## 4. Interacting with the smart function in a web application

Smart functions can provide a secure, transparent backend for web applications.
You use a browser-based wallet to encrypt (or "sign") requests so Jstz knows that they came from your account.
Follow these steps to run a sample web application that uses Jstz as a backend:

1. Build and install the Jstz development wallet in the Google Chrome browser as described here: https://github.com/jstz-dev/dev-wallet.

1. Create an account in the wallet by clicking the extension icon and then clicking **Generate new wallet**.

1. On the same system as you are running the Jstz sandbox, build and run the `web-call-to-jstz` sample application:

   ```sh
   git clone https://github.com/jstz-dev/dev-wallet
   cd dev-wallet/examples/web-call-to-jstz
   pnpm i
   pnpm dev
   ```
...
1. Paste the address of the smart function into the web application.
...
1. In the extension, click **Sign** to sign the transaction.
   ...
   ![The web application showing the response from a successful call to the sample smart function](/img/quick_start_web_app.png)
