...
- A `handler` function.
  ...
  The handler function receives a Jstz HTTP [request](/api/request) object that includes the address of the account that called it in its `Referer` header and an optional request body.
...
1.  Compile and deploy the smart function to the sandbox:
    ...
    After you deploy the smart function, you cannot stop it or change it.
