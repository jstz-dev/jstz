...
If a smart function has a balance of tez, it can send tez to a user account or smart function as part of a request by putting the amount of mutez in the `X-JSTZ-TRANSFER` header in requests, as in this example:
...
const send_request = new Request(`tezos://${smartFunctionB}`, {
  headers: {
...
To send tez to a smart function without calling the smart function and running its handler function, send the tez in a request to `tezos://<ADDRESS>/-/noop`, as in this example:

```typescript
const ONE_TEZ = 1000000;
const call_request = new Request(`tezos://${smart_function}/-/noop`, {
  headers: {
...
## Including tez in a response

A smart function can include tez in a response by setting the `X-JSTZ-TRANSFER` header in the response.
...
You can see how many mutez are included in a request by checking the `X-JSTZ-AMOUNT` header, which includes the amount as a string:
