...
You can use the local sandbox to test smart functions in a simulated environment before you deploy them to a persistent Jstz network.

1. Build the smart function as described in [Building smart functions](/functions/building).

1. Start the Jstz sandbox by running `jstz sandbox --container start` or by starting the [`jstzd`](/jstzd) daemon directly.
