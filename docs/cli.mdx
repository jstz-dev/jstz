...
Bridge commands require the Octez client; see [The Octez client](https://docs.tezos.com/developing/octez-client) on docs.tezos.com.
...
The `--name` argument sets a local alias for the smart function in the specified network.
...
All other clients, including web applications, must use the address of the function instead of this alias.

Also, aliases are tied to a specific network.
If you deploy a function with a specific alias to one network and then deploy another function with the same alias to a different network, the second alias overwrites the first alias.
...
Starts a REPL environment for interactive experimentation and testing of smart functions.
